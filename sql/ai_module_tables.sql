...
CREATE TABLE `ai_chat_role` (
  ...
  `knowledge_ids` TEXT DEFAULT NULL COMMENT '引用的知识库编号列表，关联 ai_knowledge.id',
  `tool_ids` TEXT DEFAULT NULL COMMENT '引用的工具编号列表，关联 ai_tool.id',
  `mcp_client_names` TEXT DEFAULT NULL COMMENT '引用的 MCP Client 名字列表',
...
CREATE TABLE `ai_chat_message` (
  ...
  `segment_ids` TEXT DEFAULT NULL COMMENT '知识库段落编号数组，关联 ai_knowledge_segment.id',
  `web_search_pages` TEXT DEFAULT NULL COMMENT '联网搜索的网页内容数组',
  `attachment_urls` TEXT DEFAULT NULL COMMENT '附件 URL 数组',
  `create_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '最后更新时间',
  `creator` VARCHAR(255) DEFAULT NULL COMMENT '创建者',
  `updater` VARCHAR(255) DEFAULT NULL COMMENT '更新者',
...
CREATE TABLE `ai_image` (
  ...
  `options` TEXT DEFAULT NULL COMMENT '绘制参数，不同 platform 的不同参数',
  `buttons` TEXT DEFAULT NULL COMMENT 'mj buttons 按钮',
